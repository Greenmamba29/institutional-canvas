 ...
 * Supabase-only — Airtable removed from critical path to avoid load failures
 */
...
function getEsgStatus(purityLevel: string | null): { label: string; ready: boolean } {
  if (!purityLevel) return { label: 'ESG Pending', ready: false };
    ...
    <>
      <div className="p-6 space-y-6 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <StatCard icon={ShieldCheck} value={filteredListings.length} label="CoC Tracked" colorClass="bg-secondary text-foreground" />
        </div>
